import type { DiseaseSubPageContent } from "@/lib/types";

/**
 * Source: dr-anavil-step7-missing-pages-2026-07-12.docx, Page 3.
 *
 * The FAQ "permanently cure vitiligo?" answer below uses the fixed wording
 * from dr-anavil-complete-legal-audit-2026-07-14.docx FLAG 3 rather than
 * the doc's original phrasing — this page repeats the exact same flagged
 * question, so the same fix applies here pre-emptively rather than
 * shipping known-flagged language and correcting it later.
 */
export const VITILIGO_PAGE: DiseaseSubPageContent = {
  slug: "vitiligo-treatment-jaipur",
  parentSlug: "skin-diseases",
  parentLabel: "Skin Diseases",
  pageTitle: "Vitiligo Treatment in Jaipur | Homeopathy for Leucoderma | Yadav Homeo Clinic",
  metaDescription:
    "Homeopathic vitiligo treatment at Yadav Homeo Clinic Jaipur — 30+ years, 1000+ vitiligo cases treated. Constitutional approach. Repigmentation seen in 40-60% of cases. Book consultation.",
  focusKeyword: "vitiligo treatment in Jaipur",
  secondaryKeywords: [
    "leucoderma treatment Jaipur",
    "homeopathy for vitiligo",
    "safed daag ka ilaj",
    "vitiligo repigmentation homeopathy Jaipur",
    "best vitiligo doctor Jaipur",
  ],
  hero: {
    headline: "Vitiligo — When the Spread Stops, Healing Can Begin.",
    subheadline:
      "You have watched the white patches appear, spread, and affect parts of your face and body that have changed how you see yourself in the mirror. Three things need to happen: the spread must stop, the immune attack on melanocytes must be addressed, and wherever possible — the pigment must return. That is exactly what 30 years of classical homeopathic treatment at Yadav Homeo Clinic aims to do.",
    trustLine:
      "30+ Years of Vitiligo Cases | 1000+ Vitiligo Patients Treated | Repigmentation Seen in 40–60% of Cases | Constitutional — No Topical Steroids",
  },
  sections: [
    {
      heading: "Understanding Why Vitiligo Spreads — and Why Creams Cannot Stop It",
      navLabel: "Why It Spreads",
      paragraphs: [
        "Vitiligo — called safed daag or leucoderma in Hindi — occurs when the immune system mistakenly destroys melanocytes, the cells responsible for producing skin pigment. The result is white patches that appear unpredictably, spread without warning, and affect the face, hands, elbows, knees, lips, and other body areas most visibly.",
        "The question patients always ask is: why does it keep spreading even when I am using creams?",
        "The answer is that creams — steroid-based, tacrolimus-based, or otherwise — address the skin from outside. They can sometimes slow local progression or even stimulate some repigmentation in a treated area. But they do not address why the immune system is attacking melanocytes in the first place. As soon as the cream is stopped, or in untreated areas, the destruction continues.",
        "Constitutional homeopathy works at the level where vitiligo actually originates — the immune dysregulation. By identifying and correcting the specific immune imbalance in the individual patient, it aims to stop the destruction at its source. When this happens, the skin's own melanocytes — wherever they survive — can begin functioning again. Repigmentation follows not because we applied something to the skin, but because the body's own intelligence was restored.",
      ],
      media: { type: "video", caption: "Dr. Anavil explains: why vitiligo spreads and why creams alone can't stop it" },
    },
    {
      heading: "What Our 30 Years of Vitiligo Cases Actually Show — No Exaggeration",
      navLabel: "What We See",
      paragraphs: ["Based on our clinical experience at Yadav Homeo Clinic across hundreds of vitiligo cases:"],
      listStyle: "stats",
      list: [
        "Halting of Spread — seen in the majority of patients who complete a full course of constitutional treatment. This is the most consistent and reliable outcome. New patches stop appearing. Existing patches stabilise.",
        "Visible Repigmentation — seen in approximately 40 to 60 percent of patients who complete 12 or more months of treatment. Repigmentation typically begins as small brown pigment dots (called 'pigment islands') appearing inside the white patches, which gradually enlarge and coalesce. It is visible — patients can photograph and track the progress.",
        "Complete or Near-Complete Clearance — seen in a smaller group, typically patients with early-stage vitiligo (under 3 years), actively spreading patches, involvement of the face and trunk, and younger age. These are our most remarkable cases — and they are genuinely life-changing.",
        "Limited Response — some cases respond less dramatically: long-standing patches of more than 10–15 years, patches at fingertips or lips, patients with multiple concurrent autoimmune conditions, or older age with extensive coverage. We will tell you honestly at the first consultation which category your case falls into.",
      ],
      note: "Important qualifier: all percentage estimates above are based on clinical experience at Yadav Homeo Clinic. They are not derived from controlled clinical trials. Individual results vary based on disease duration, extent, immune status, compliance, and constitutional factors. ‘In our clinical experience’ refers to our practice data, not scientific trial data.",
    },
    {
      heading: "Vitiligo Treatment Timeline: What to Expect Month by Month",
      navLabel: "Timeline",
      subsections: [
        {
          label: "Months 1–2",
          paragraphs: [
            "Constitutional assessment and prescription. Primary goal is halting spread. Patient may notice less itching at patch edges (a sign of activity stopping).",
          ],
        },
        {
          label: "Months 2–4",
          paragraphs: [
            "Monitoring. Most patients see spread halt in this window. Some early repigmentation cases show first pigment dots by Month 3.",
          ],
        },
        {
          label: "Months 4–8",
          paragraphs: [
            "For responding cases, pigment islands become visible and begin to coalesce. Speed varies — face patches often respond faster than hand and elbow patches.",
          ],
        },
        {
          label: "Months 8–14",
          paragraphs: [
            "Continued repigmentation for responding cases. Prescription adjustments based on response patterns. Some patients require a change of remedy at this stage to drive deeper.",
          ],
        },
        {
          label: "Months 14+",
          paragraphs: [
            "For extensive cases, treatment continues. For early-stage cases with good response, treatment may be completing. Follow-up monitoring is recommended for 6 months after stopping.",
          ],
        },
      ],
    },
    {
      heading: "Types of Vitiligo We Treat",
      navLabel: "Types We Treat",
      listStyle: "cards",
      list: [
        "Segmental Vitiligo — patches confined to one body segment. Often responds earlier and more completely to constitutional treatment.",
        "Non-segmental (Generalised) Vitiligo — the most common type. Bilateral, often symmetrical. Variable response depending on stage and duration.",
        "Facial Vitiligo — patches on face, lips, eyelids. Good response area when treated early. High impact on patient confidence — these cases are our priority.",
        "Acrofacial Vitiligo — fingertips and face. More challenging — fingertip patches are among the most resistant in any treatment system.",
        "Universal Vitiligo — extensive coverage. Constitutional treatment can stabilise; full repigmentation goals are realistic only for newer areas.",
        "Vitiligo with other autoimmune conditions — thyroid disorders, alopecia areata, PCOS. Constitutional treatment addresses all simultaneously.",
      ],
    },
    {
      heading: "Why Yadav Homeo Clinic for Vitiligo?",
      navLabel: "Why Us",
      media: { type: "photo", caption: "Constitutional case-taking session, in progress" },
      paragraphs: [
        "Our vitiligo experience is the deepest part of our clinical identity. Dr T P Yadav began treating vitiligo cases in 1991 when Jaipur had very few classical homeopaths. For 30 years, patients with vitiligo have come from across Rajasthan, from Delhi, from abroad — many of them referred by other patients who recovered here.",
        "Dr Anavil Yadav now leads vitiligo cases with the same constitutional approach — deepened by his own clinical training and updated by his engagement with current immunological understanding of the disease.",
        "What makes our approach different is not a specific medicine or a protocol. It is the commitment to finding the constitutional remedy that matches the individual — their physical constitution, their emotional tendencies, their reaction patterns, their immune fingerprint. Vitiligo is not treated the same way in every patient here. It is treated as the individual patient's immune system speaking — and answered accordingly.",
      ],
    },
  ],
  patientStory: {
    quote:
      "Six years of watching the white patches spread across my face and hands. Every morning checking the mirror for new spots. Three dermatologists. Two other homeopaths. PUVA therapy twice a week for 8 months. Nothing stopped it. I came to Yadav Homeo Clinic with almost no hope left — just wanting to try one more thing. The consultation was unlike any other — an hour of questions that went far beyond my skin. Within 3 months the spreading stopped. By the 7th month I saw the first pigment dots returning to my cheek patch. I photographed every change. It has been 14 months now. My face has shown substantial repigmentation. My hands are responding more slowly — but they are responding. I do not have words for what this means.",
    attribution: "Patient, Delhi — Vitiligo, Face and Hands, 14 months of treatment",
    note: "Individual results vary. This patient's outcome may not be typical.",
  },
  faqs: [
    {
      question: "Can homeopathy permanently cure vitiligo?",
      answer:
        "In many cases — particularly early-stage vitiligo treated consistently for 12–18 months — lasting repigmentation is achievable. Many patients experience improvement that does not return after treatment ends. However, outcomes vary significantly and cannot be guaranteed for every individual.",
    },
    {
      question: "How long does vitiligo treatment take to show results?",
      answer:
        "Halting of spread is typically seen in the first 2–3 months of correct constitutional treatment. First signs of repigmentation — small pigment dots inside patches — usually appear between months 3 and 6 in responding cases. Full repigmentation of a patch can take 12–24 months. Face patches respond faster than hand and fingertip patches.",
    },
    {
      question: "Do I need to stop my steroid cream or PUVA therapy before starting?",
      answer:
        "No. Never stop any prescribed treatment abruptly. You can continue your existing treatment alongside homeopathic constitutional treatment. As your condition responds, you will naturally need the external treatment less — and can taper it gradually under your dermatologist's guidance.",
    },
    {
      question: "Is vitiligo treatment at Yadav Homeo Clinic available online?",
      answer:
        "Yes. We treat vitiligo patients from across India and internationally through our online consultation process. You send clear photographs of patches in natural light alongside your case history. Constitutional prescription is based on your individual totality — which can be gathered accurately online. Online outcomes for vitiligo are comparable to in-clinic.",
    },
    {
      question: "My vitiligo is 15 years old and covers large areas. Is it too late?",
      answer:
        "It is never too late to try. Long-standing or extensive vitiligo is more challenging — and we will tell you honestly. But even in cases of 10–15 years duration, spread can be halted and some repigmentation achieved in newer or more recently active patches. We have had meaningful responses in cases that appeared to have no viable melanocytes remaining.",
    },
    {
      question: "What is the consultation fee for vitiligo?",
      answer:
        "Please WhatsApp us at +91-8949427254 for current consultation fees. In-clinic and online fees are available. Package options for longer treatment courses are discussed at the first consultation.",
    },
  ],
  finalCta: "If You Have Vitiligo — Your First Step Is a Conversation. Book Your Consultation.",
  disclaimer:
    "This page provides general information about how classical homeopathy may support vitiligo treatment. Individual results vary and are not guaranteed. It is not a substitute for professional dermatological diagnosis or ongoing specialist care — always continue prescribed treatment and consult your treating dermatologist before making any changes.",
  aboutCondition: {
    name: "Vitiligo",
    alternateNames: ["Leucoderma", "Safed Daag"],
    description:
      "A chronic autoimmune skin condition in which melanocytes are destroyed by the immune system, causing white patches on the skin, treated here with constitutional homeopathic treatment.",
  },
};
